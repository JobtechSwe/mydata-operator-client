...
async function allKeys (config) {
  const serClientKey = serializeKey(config.clientKeys.publicKey, 'client_key', 'sig')
  const signKeys = await config.signKeyProvider()
  const serSignKeys = signKeys.map(({kid, key}) => serializeKey(key, kid, 'sig'))
  return {
    keys: [
      serClientKey,
      ...serSignKeys
