...
async function request (client, consentRequestDescription) {
  ...
  const encKey = await client.keyProvider.generateTemp({ use: 'enc' })
  const data = {
...
async function approve (client, consent) {

}

module.exports = client => ({
  request: request.bind(null, client),
  approve: approve.bind(null, client)
})
